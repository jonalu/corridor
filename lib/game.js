...
const locateWalls = state =>
  getMoves(state).filter(excludePawnMoves)
...
const isWallMove = move => move.length === 3 && ['V', 'H'].indexOf(move[0]) > -1

const isPawnMove = move => ['V', 'H'].indexOf(move[0]) === -1
