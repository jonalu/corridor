function CorridorClient (options) {
  ...
  this.wRect = this.dimensions.width / this.options.squares;  this.proposedWall = {x: null, y: null, direction: null}
}

CorridorClient.prototype = {
  constructor: CorridorClient,
  defaults: function () {
    return {
      squares: 9,
      gridStroke: '#aaa',
  ...
  drawLegalMoves: drawLegalMoves = function () {
  ...
  handleClick: function (e) {
    ...
    if(this.isValidWall(this.proposedWall) && this.posWithinRect(this.proposedWall, pointerPosition)) {
      window.location = '/move/' + this.proposedWall.squareAnnotation +
      '?state=' +
  ...
  posWithinRect: function (rect, pos) {
  ...
  resolvePointerPosition: function (e) {
    return {
      x: e.x - e.target.offsetLeft,
      y: e.y - e.target.offsetTop
    };
  },
  drawWall: function (wall, color) {
  ...
  toSquare: toSquare = function (x,y) {
